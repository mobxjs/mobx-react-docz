...
You can drop `StoreProvider` anywhere in the tree (_top is preferred_) and access store with convenient `useStore` hook. The Provider won't re-render unless forced by a parent.
