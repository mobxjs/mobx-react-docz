...
Notice that even though `useEffect` references observable value we don't treat it is such simply because it's stored there mostly for a convenience. In case you do expect its value to be changed, you would have to wrap it to `autorun`.
...
However, if the `sectionName` would be dynamic, you would need to `useAsObservableSource` again or perhaps set the fetcher result inside the store to make it observable.
